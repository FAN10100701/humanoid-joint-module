...
4. **同步登记**(漏一处会被作者退回)——跑 `node _本地工具/登记页面.js` 自动完成六处:首页卡片网格 + `SITE_SECTIONS` ids、`_assets/search-index.js` 索引、`_assets/page-meta.js` 学习目标、`sitemap.xml`、`06_学习工具/06_学习地图.html` 节点、百度提交清单;手动三件随需:页面 `prev`/`next` 翻页链、新增 `_assets/*.js` 时登记进根 `sw.js` PRECACHE、学习路径相关页进 `_assets/path-data.js`;最后在 `CHANGELOG.md` 顶部登记本期更新
...
| `_assets/ib-data-b.js`(硬件软件 9 学科) | 同上 | 同上(字段含 `extend?`) |
| `_assets/ib-data-c.js`(通用 4 学科,V2.1.6) | 同上 | 同上 |
| `_assets/ib-data-d.js`(控制工程/机电基础课 8 学科,V2.1.22) | 同上 | 同上;四文件合计科目/题数必须与 `_assets/site.js` 的 `S.STATS` 一致(自检 C3 校验) |
