...
$s = $enc.ReadFile($p); $s = [regex]::Replace($s, 'S\.VERSION\s*=\s*"V[\d.]+\([^)]*\)"', ('S.VERSION = "' + $Version + '(' + $Date + ')"')); $enc.WriteFile($p, $s, (New-Object System.Text.UTF8Encoding($false)))
...
$enc.WriteFile($p, $s, (New-Object System.Text.UTF8Encoding($false)))
